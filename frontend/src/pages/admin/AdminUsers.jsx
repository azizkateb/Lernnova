            ...
            {!loading && users.map((user) => (
              <div key={user.id} className="space-y-4 p-4">
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {user.profile_slug || user.public_id ? (
                    <Link
                      to={`/profile/${user.profile_slug || user.public_id}`}
                    ...
                    <span className="inline-flex items-center justify-center rounded-xl bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-400 dark:bg-slate-700 dark:text-slate-500">
                      {t('dashboard.admin.users.noProfile', 'No Profile')}
                    </span>
                  )}
