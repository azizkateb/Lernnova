...
const SellerDashboard = () => {
  ...
  // Parse stats from the backend response
  const totalServices = overview?.services?.total || 0;
  const activeServices = overview?.services?.active || 0;
  const totalProducts = overview?.products?.total || 0;
  const activeProducts = overview?.products?.active || 0;
  const totalOrders =
    (overview?.service_orders?.total || 0) +
    (overview?.product_orders?.total || 0);
  const totalRevenue = overview?.revenue?.total || 0;

  // Parse recent orders
  const recentServiceOrders = Array.isArray(recent?.service_orders)
    ? recent.service_orders
    : [];
  const recentProductOrders = Array.isArray(recent?.product_orders)
    ? recent.product_orders
    : [];

  // Combine recent orders for display
  ...
  const stats = [
    { title: t('dashboard.stats.totalRevenue', 'Total Revenue'), value: formatCurrency(totalRevenue), icon: DollarSign, color: 'emerald', trend: '+12.5%' },
    { title: t('dashboard.seller.totalOrders', 'Total Orders'), value: totalOrders, icon: ShoppingBag, color: 'indigo' },
    { title: t('sidebar.myServices', 'My Services'), value: totalServices, icon: FileText, color: 'sky' },
    { title: t('sidebar.myProducts', 'My Products'), value: totalProducts, icon: Package, color: 'amber' },
  ];

  // Top products from recent orders or empty array
  const topProducts = recentProductOrders
    .slice(0, 5)
    .map(order => ({
      id: order.product?.id || order.id,
      title: order.product?.title || 'Unknown Product',
      thumbnail: order.product?.thumbnail_url || order.product?.thumbnail,
                    ...
                    {allRecentOrders?.length > 0 ? (
                      allRecentOrders.map(order => (
                        <tr key={order.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors">
                           ...
                           <td className="px-6 py-4 text-sm font-medium text-slate-600 dark:text-slate-300 line-clamp-1 max-w-[200px]">{order.service?.title || order.product?.title || 'N/A'}</td>
