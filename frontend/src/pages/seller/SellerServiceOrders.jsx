...
const SellerServiceOrders = () => {
  ...
  /* ─── Effects ────────────────────────────────────────── */
  // Status filter triggers refetch on page 1
  useEffect(() => {
    ...
    <div className="space-y-8">
      ...
      <div className="relative">
        ...
        <h1 className="text-3xl font-light text-slate-900 dark:text-white tracking-tight mb-1">
          {t('pages.seller.serviceOrders.title', 'Incoming Service')}{' '}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          ...
          <select
            ...
            className="appearance-none pl-9 pr-8 py-2.5 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400 transition cursor-pointer"
          >
        ...
        {/* Result counter */}
        <div className="hidden sm:flex items-center px-3 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
      ...
      {/* Inline error banner (when partial data already loaded) */}
              ...
              <tr className="border-b border-gray-100 dark:border-gray-700 bg-slate-50 dark:bg-slate-900/30">
                <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    {t('pages.seller.serviceOrders.col.orderId', 'Order ID')}
                  </span>
                </th>
                <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  <span className="inline-flex items-center gap-1.5">
                    <Briefcase className="w-3.5 h-3.5" />
                    {t('pages.seller.serviceOrders.col.service', 'Service')}
                  </span>
                </th>
                <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  <span className="inline-flex items-center gap-1.5">
                    <User className="w-3.5 h-3.5" />
                    {t('pages.seller.serviceOrders.col.buyer', 'Buyer')}
                  </span>
                </th>
                <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  <span className="inline-flex items-center gap-1.5">
                    <Coins className="w-3.5 h-3.5" />
                    {t('pages.seller.serviceOrders.col.amount', 'Amount')}
                  </span>
                </th>
                <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  <span className="inline-flex items-center gap-1.5">
                    <Activity className="w-3.5 h-3.5" />
                    {t('pages.seller.serviceOrders.col.status', 'Status')}
                  </span>
                </th>
                <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  <span className="inline-flex items-center gap-1.5">
                    <CalendarDays className="w-3.5 h-3.5" />
                    {t('pages.seller.serviceOrders.col.created', 'Created')}
                  </span>
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700/50">
              {/* Loading skeletons (when refetching with existing data) */}
              ...
              {!loading && filteredOrders.map(order => (
                <tr
                  ...
                  className="group hover:bg-emerald-50/40 dark:hover:bg-emerald-900/10 transition-colors cursor-pointer"
                >
                  {/* Order # */}
                  <td className="px-6 py-4">
                    <span className="font-mono text-xs text-slate-500 dark:text-slate-400 group-hover:text-emerald-700 dark:group-hover:text-emerald-300 transition-colors">
                      #{order.id}
                    </span>
                  </td>
                  ...
                  {/* Service */}
                  <td className="px-6 py-4">
                    <p className="font-semibold text-slate-900 dark:text-white truncate max-w-[260px]">
                      {order.service?.title ||
                      ...
                      <EmptyState
                        ...
                        title={
                          search || statusFilter
                            ...
                            : t(
                                'pages.seller.serviceOrders.emptyTitle',
                        ...
                        description={
                          search || statusFilter
                            ...
                            : t(
                                'pages.seller.serviceOrders.emptyDesc',
                                'Orders from buyers will appear here as soon as they purchase one of your services.'
                              )
                        }
                      />
                    </div>
