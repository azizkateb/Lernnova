  ...
  const fetchOrders = useCallback(async (overridePage) => {
    ...
    try {
      ...
      setError(
        ...
        err.message ||
        t('dashboard.seller.loadingProductOrders', 'Failed to load product orders')
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400 font-medium">
            {t('dashboard.seller.noPermission', 'You do not have permission to view this page.')}
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-8">
      ...
      <div>
        <span className="bg-emerald-600 dark:bg-emerald-500 text-white text-[9px] font-bold px-2 py-0.5 rounded uppercase mb-2 inline-block tracking-widest">
          {t('dashboard.seller.badge', 'Seller')}
        </span>
        <h1 className="text-3xl font-light text-slate-900 dark:text-white tracking-tight mb-1">
          {t('dashboard.seller.myProductOrders', 'My Product')}{' '}
          <span className="font-serif italic text-emerald-600">
            {t('dashboard.seller.myProductOrdersAccent', 'Orders')}
          </span>
        </h1>
        <p className="text-slate-500 dark:text-slate-400 font-medium">
          {t(
            'dashboard.seller.myProductOrdersSubtitle',
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder={t(
              'dashboard.seller.searchProductOrders',
        ...
        >
          <option value="">{t('dashboard.seller.allPaymentStatuses', 'All Payment Statuses')}</option>
        ...
        <select
          ...
          className="px-3 py-2.5 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-400 transition cursor-pointer"
        >
          <option value="">{t('dashboard.seller.allOrderStatuses', 'All Order Statuses')}</option>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            >
              {t('dashboard.seller.retry', 'Retry')}
            </button>
          </div>
        )}
              ...
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700 bg-slate-50 dark:bg-slate-900/30">
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.orderNumber', 'Order #')}
                  </th>
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.productColumn', 'Product')}
                  </th>
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.buyer', 'Buyer')}
                  </th>
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.amount', 'Amount')}
                  </th>
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.paymentMethod', 'Payment Method')}
                  </th>
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.paymentStatus', 'Payment Status')}
                  </th>
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.orderStatus', 'Order Status')}
                  </th>
                  <th className="px-6 py-3.5 text-left text-[11px] font-bold uppercase tracking-widest text-slate-400">
                    {t('dashboard.seller.createdColumn', 'Created')}
                  </th>
                </tr>
              </thead>
                ...
                {!loading && filteredOrders.map(order => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      {order.buyer?.email && (
                        <p className="text-slate-400 dark:text-slate-500 text-xs mt-0.5 truncate max-w-[200px]">
                          {order.buyer.email}
                        </p>
                      )}
                    </td>
                  ...
                  <tr>
                    <td colSpan={8} className="py-20 text-center">
                      ...
                      <p className="text-slate-600 dark:text-slate-400 font-semibold">
                        {t('dashboard.seller.noProductOrdersFound', 'No product orders found')}
                      </p>
                      <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">
                        {t(
                          'dashboard.seller.noProductOrdersSubtitle',
        ...
        {!loading && !error && pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 dark:border-gray-700">
            ...
            >
              ...
              {t('dashboard.seller.previous', 'Previous')}
            </button>

            <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">
              {`${t('dashboard.seller.page', 'Page')} ${pagination.page} ${t('dashboard.seller.of', 'of')} ${pagination.totalPages}`}
            </span>
